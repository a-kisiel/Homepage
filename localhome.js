...
function bakeCookie(cooKey, value) {
    localStorage.setItem(cooKey, value)
}

// Checks whether a given cookie exists
function isCookie(cooKey) {
    return (localStorage.getItem(cooKey) != null)
}

// Gets a value from an existing cookie using a key
function getCookie(cooKey) {
    return localStorage.getItem(cooKey)
}
...
is24 = (getCookie('ac') === 'active') ? true : false;
voiceOn = (getCookie('vc') === 'active') ? true : false;

console.log("AC Cookie: " + getCookie('ac'))
console.log("VC Cookie: " + getCookie('vc'))
